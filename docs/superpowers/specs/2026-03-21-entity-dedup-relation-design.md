...
1. **消歧优先于合并** — 名称只是线索，不是结论。先判断"这是哪个 `Entity_X`"，再决定"要不要合并"
2. **证据分级，置信度驱动** — 低置信度不合并，只建议；高置信度才自动执行
3. **文章上下文是元数据** — 实体不孤立存在，它依附于文章来源，合并建议必须携带上下文解释

### 2.2 关键洞察：同名称实体消歧

> 同一名称在不同文章中可能指代完全不同的实体：
> - 文章 A 的 `Entity_A` = 全球性主体（外资品牌/跨国集团）
> - 文章 B 的 `Entity_A` = 本地化主体（合资公司/区域子公司）
>
> 如果仅凭名称相似度合并，会把两个不同实体错误合并，或把同一实体当作两个

**设计影响：** 合并建议必须携带来源上下文 + 消歧信号，不能仅靠名称相似度。**典型场景包括但不限于**：跨国品牌简称（如 "BrandX" vs "BrandX Co."）、合资公司简称（如 "合营A" vs "A-B合营公司"）、同一品牌在不同市场的不同法律主体。
...
**多标签示例：**
- "全球集团A" = 主类型:集团 + 标签:公司,品牌,赛道
- "集团A子公司" = 主类型:公司 + 标签:集团成员
- "品牌B有限公司" = 主类型:公司 + 标签:品牌,行业
- "合资品牌C" = 主类型:公司 + 标签:品牌,合资

**重要区分：集团层级关系 vs 多标签**

> "集团A"和"集团A子公司"是**两个不同实体**，它们的关系应通过 `entity_relation(from=集团A, to=集团A子公司, type=SUBSIDIARY)` 建模，而不是用多标签合并到一个实体上。

- **多标签**：描述单个实体的多重属性（如"集团A"既是集团又是品牌）
...
def infer_entity_type(name: str, fact_type: str = "") -> tuple[str, list[str]]:
    ...
    例如：输入"集团A"，返回 ("GROUP", ["company", "brand", "market"])
    """
    ...
```
...
def disambiguate(name: str, context: DisambiguateContext = None) -> DisambiguationResult:
    ...
    有上下文时（context 非空）：
      ...
      - ambiguity_note: 消歧解释（如"需注意避免与同名合资公司混淆"）
...
```
输入：候选实体 A ("品牌X") 和候选实体 B ("Brand_X Co.")

证据收集：
├─ 名称证据 (0.7)     — 规范化后相似，但"品牌X"是简称，不足以确认
├─ 事实共现 (0.85)   — 两个实体在多篇文章中共同出现，predicate 一致
├─ 网络证据 (0.9)     — 搜索"品牌X Brand_X"返回明确关联
└─ 消歧上下文 (0.8)   — A 的 qualifier 含"全球"、"跨国"，B 含"有限公司"，指向同一实体

LLM 综合判断：
→ "A 是 Brand_X Co. 在中国市场报道中的简称"
→ confidence: 0.88
→ ambiguity_note: "需注意避免与同名合资公司混淆"
→ 决策：建议人工确认
```
...
- `ambiguity_note` — 当名称存在多义性时，记录消歧解释（如"需注意避免与同名合资公司混淆"）
...
3. **消歧可用** — `disambiguate("品牌X")` 能返回多个候选 + 消歧解释，无上下文时 fallback_action = "require_manual"
